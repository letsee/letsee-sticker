...
class News extends Component {
  ...
  componentWillMount() {
    this.setState({ loading: true }, () => {
      // get firebase db reference for news
      ...
      ref.limitToLast(PER_PAGE + 1).once('value', (snapshot) => {
        ...
        if (dataSize > 0) { // set pagination cursors
        ...
        this.setState(prevState => ({
          ...
          // listen to and add any new data to the list
  ...
  handleChildRemoved = (oldChildSnapshopt) => { // remove from list if removed from firebase
    this.setState((prevState) => {
      const {
  ...
  handleChildChanged = (childSnapshot) => { // update list item if updated from firebase
